"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) {
            if (e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]))
                t[p[i]] = s[p[i]];
        }
    return t;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Volume = void 0;
class Volume {
    constructor(client) {
        this.client = client;
    }
    listVolumes(app_name) {
        return __awaiter(this, void 0, void 0, function* () {
            const path = `apps/${app_name}/volumes`;
            return yield this.client.restOrThrow(path);
        });
    }
    getVolume(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            const { app_name, volume_id } = payload;
            const path = `apps/${app_name}/volumes/${volume_id}`;
            return yield this.client.restOrThrow(path);
        });
    }
    createVolume(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            const { app_name } = payload, body = __rest(payload, ["app_name"]);
            const path = `apps/${app_name}/volumes`;
            return yield this.client.restOrThrow(path, 'POST', body);
        });
    }
    deleteVolume(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            const { app_name, volume_id } = payload;
            const path = `apps/${app_name}/volumes/${volume_id}`;
            return yield this.client.restOrThrow(path, 'DELETE');
        });
    }
    extendVolume(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            const { app_name, volume_id } = payload, body = __rest(payload, ["app_name", "volume_id"]);
            const path = `apps/${app_name}/volumes/${volume_id}/extend`;
            return yield this.client.restOrThrow(path, 'PUT', body);
        });
    }
    listSnapshots(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            const { app_name, volume_id } = payload;
            const path = `apps/${app_name}/volumes/${volume_id}/snapshots`;
            return yield this.client.restOrThrow(path);
        });
    }
}
exports.Volume = Volume;
